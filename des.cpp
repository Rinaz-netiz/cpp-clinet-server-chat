...
namespace des {
    ...
    public:
        ...
        void IP();
        void PermChoice1(vector<int> &key);
        void PermChoice2();
        void Expansion();
        void inverse();
        void xor_two();
        ...
        void substitution();
        void permutation();
        void keygen(vector<int> &key);
        char * Encrypt(const char *, vector<int> &key);
        char * Decrypt(const char *, vector<int> &key);
    };
    ...
    void key_to_binary(vector<int> &key, string& password){
        int temp[8];
    ...
    void Des::PermChoice1(vector<int> &key) // Выбор перестановки-1: преобразует ключ из 64 бит в 56
    {
        int k = 57, i;
        for (i = 0; i<28; i++)
        {
    ...
    void Des::keygen(vector<int> &key)// Генерирует ключи для каждого раунда шифрования
    {
        PermChoice1(key);
    ...
    char * Des::Encrypt(const char *Text1, vector<int> &key)
    {
    ...
    char * Des::Decrypt(const char *Text1, vector<int> &key)
    {
        int i, a1, j, nB, m, iB, k, K, B[8], n, t, d, round;
        char *Text = new char[1000];
        unsigned char ch;
